 ...
 ***********/
	 ...
	 */
	 private function findContentLoc(){
		if(isset($this->ebookData->opfPath) && dirname($this->ebookData->opfPath)!="."){
			$this->ebookData->contentFolder = dirname($this->ebookData->opfPath)."/";
		}else if (!isset($this->ebookData->opfPath)) {
	...
	private function readEpubFile($zipFile, $fileLocation){
		...
		if($ext == "epub" && is_file($zipFile)){
			...
			if ($zip) {
			  while ($zip_entry = zip_read($zip)) {
			    	if(zip_entry_name($zip_entry) == $fileLocation && zip_entry_open($zip, $zip_entry))	{
